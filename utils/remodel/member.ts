  ...
  newAsmat: {
    params: {
      memberRegister(payload) {
        const {
          phoneNumb, name, gender, email, work, birthPlace,
          birthDate, address, city, posCode, idNumber, memberType, expiredDate, isActive
        } = payload;
    ...
    response: {
      ...
      memberType(payload) {
        ...
        const remodelData = payload.tiketux.result.map(({
          nama: name,
          kode: code
        }) => ({
          name,
