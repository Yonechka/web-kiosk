<script setup>
  ...
  const { checkPaymentStatus, startIntervalCheckPayment } = useCheckPayment(router, t, false); // isNewAsmat
  ...
  /* EDC */
  const { isEdc, showSpinnerEdc, closeEdcModal, edcModal } = useEDC(router, t)
  ...
  /* onUnmounted Hook */
  onUnmounted(() => {
    console.log('onUnmounted payment hook called');
...
<template>
    ...
    <div v-if="!isShow" class="mt-[40px] rounded-[35px] w-full mb-[224.71px]" :class="`bg-[${colors.secondaryColor}]`">
        ...
        <div class="flex pl-[51px] pr-[51px] pt-[30px] pb-[25px] items-center justify-center">
            ...
            >
                <a href="#" class="font-bold text-[#FFFFFF] text-xl">
                  {{ $t('payment.howToPay') }}
                </a>
            </div>
            <div
              ...
              :class="`bg-[${colors.accentColor}]`"
            >
                <div class="font-bold text-xl" :class="`text-[${colors.secondaryColor}]`">
