...
<template>
 <div class="mt-[67px] rounded-[35px] w-full h-[auto] md:grid-cols-6 " :class="`bg-[${colors.secondaryColor}]`">
    <div class="ml-[34px] py-[37px] leading-[41.58px] pr-[41.25px]">
        <div class=" font-medium text-[36px]  " :class="`text-[${colors.fontPrimaryColor}]`">Asal Keberangkatan</div>
        <div v-if="scheduleStore.departurePools" class="flex justify-between pt-[31px] -mt-[25px]">
    ...
    <Transition :duration="350" name="nested">
      <div v-show="isShow" class="w-full h-full absolute top-0 flex justify-end items-center z-[100]">
        ...
        <div class="flex inner z-[99]">
            <div @click="openModal" class="bg-[#C66C6C] cursor-pointer absolute ml-[-140px] self-center rounded-full w-[98px] h-[98px]">
            ...
            :class="`bg-[${colors.secondaryColor}]`">
              <div class="text-[36px]" :class="`text-[${colors.fontPrimaryColor}]`">Pilih asal Keberangkatan</div>
              ...
              <div 
                ...
                class="flex cursor-pointer justify-start mt-[34px] ml-[114px]"
                @click="selectPool(pool)">
                  ...
                  <div class="text-[32px] font-semibold ml-[29px]" :class="`text-[${colors.fontPrimaryColor}]`">{{ pool.pool_name }} <br>
                      <span v-if="indexParent === 0 && index === 0" class="text-[26px]" :class="`text-[${colors.fontSecondaryColor200}]`">
                        Lokasi Saat Ini
                      </span>
                      <span v-else class="text-[26px]" :class="`text-[${colors.fontSecondaryColor200}]`">
                        {{ pool.pool_address }}
                      </span>
                  </div>
