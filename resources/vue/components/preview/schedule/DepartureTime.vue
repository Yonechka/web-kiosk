<script setup>
  ...
  const { departureTime, date } = storeToRefs(scheduleStore);
    ...
    <div class="overflow-hidden rounded-[35px] mt-[24px] relative">
        ...
        >
            <div class="flex justify-between pt-[28px]">
                <p class="text-[36px] pl-[38px]" :class="`text-[${colors.fontPrimaryColor}]`">Pilih Jam Pergi</p>
                <img @click="departureTimeModal" src="./../../../assets/images/vector.png" class="h-[36.66px] w-[36.66px]">
            </div>
            <div class="row mt-[23px] h-full relative">
                ...
                <div v-if="departureTime.length > 0" class="grid grid-cols-3 gap-x-[20px] gap-y-[16px] pb-[30px] ml-[38px]">
                    ...
                    >
                        <div v-if="isToday">
                            <div v-if="index === 0" class="h-[35px] w-[149px] bg-[#17706F] pb-[22px] mt-[10px] ml-[17px] leading-[23px] rounded-[8px]">
                                <p class=" text-[#FFFFFF] text-[20px]  py-[6px] text-center">Segera Pergi</p>
                            </div>
                        </div>
                        <div class="text-center ">
                            <span class="text-[46px] font-bold" :class="`text-[${colors.primaryColor200}]`">{{ item.departure_time }}</span><br>
                            <span
                              class="mt-[11px] text-[24px] stok"
                              :class="`text-[${colors.fontSecondaryColor400}]`"
                            >
                              Sisa
                              <strong class="mx-1" :class="`text-[${colors.fontPrimaryColor}]`"> {{ item.available_seat }}</strong>
                              Kursi
                            </span><br>
                            ...
                            <div v-if="item.promo" class="flex flex-col">
                                ...
                                <div class="text-[20px] font-bold" :class="`text-[${colors.primaryColor}]`">
                            ...
                            <div v-else
                              class="mt-[14px] text-[20px] font-bold"
                ...
                <div v-else class="w-full h-full flex justify-center items-center">
                    <div class="text-[30px]">Jadwal tidak ditemukan!</div>
                </div>
            </div>
        </div>
    </div>
      ...
      <Transition :duration="350" name="nested">   
        <div v-show="isShow" class="w-full h-full absolute top-0 flex justify-end items-center">
            ...
            <div class="flex inner z-[99] h-full items-center">
                <div @click="departureTimeModal" class="bg-[#C66C6C] flex justify-center items-center z-[100] cursor-pointer absolute ml-[-140px] self-center rounded-full w-[98px] h-[98px]">
                ...
                <div
                  ...
                  :class="`bg-[${colors.fontSecondaryColor}]`"
                >
                    <div class="text-[36px] mb-[50px]" :class="`text-[${colors.fontPrimaryColor}]`">Pilih Jam Pergi</div>
                    <div class="grid grid-cols-3 gap-x-[20px] gap-y-[16px]  ml-[38px]">
                        <div
                          v-for="(item, index) in departureTime"
                          :key="item.code"
                          @click="selectDepartureTime(item.code)"
                          class="cursor-pointer w-[181px] h-[250px] flex flex-col justify-center mr-[20px] rounded-[12px]"
                          :class="`bg-[${colors.primaryColor20}]`"
                        >
                            <div v-if="isToday">
                                <div v-if="index === 0" class="h-[35px] w-[149px] bg-[#17706F] pb-[22px] mt-[10px] ml-[17px] leading-[23px] rounded-[8px]">
                                    <p class="text-[#FFFFFF] text-[14px]  py-[6px] text-center">Segera Pergi</p>
                                </div>
                            </div>
                            <div class="text-center ">
                                <span class="text-[46px] font-bold" :class="`text-[${colors.primaryColor200}]`">
                                  {{ item.departure_time }}
                                </span><br>
                                <span
                                  class="mt-[11px] text-[24px] stok"
