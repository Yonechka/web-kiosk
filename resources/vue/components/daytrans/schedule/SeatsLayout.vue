<script setup>
  ...
  /* Initial Data */
  const { isSeatsShow, isDepTimeShow } = useInitialData();
  ...
  const confirmSelectSeatHandler = () => {
    const roundTripHandler = () => {
      isSeatsShow.value = false;
      isDepTimeShow.value = true;
    }
    scheduleStore.isRoundedTrip ? roundTripHandler() : toReservation();
  }
      ...
      <div class="rounded-[35px] w-[645px] h-[1686px] relative" :class="`bg-[${colors.secondaryColor}]`">
          ...
          <div class="absolute bottom-[40px] w-full">
            ...
            <div v-if="selectedSeats.length" class=" pl-[37px] mb-8">
              ...
              <div 
                :data-subtotal-ticket="subTotalTicketNewAsmat"
                class="font-semibold text-[32px]"
                :class="`text-${colors.fontPrimaryColor}`"
              >
                  Rp. {{ formattingNumber(subTotalTicketNewAsmat) }}
              </div>
            </div>
