...
import Spinner from '@/components/jackal/OverlaySpinner.vue';
    ...
    <div>
      ...
      <template v-else>
        <div :class="`text-[${colors.fontPrimaryColor}]`" class=" font-semibold text-2xl px-3 py-[6px] rounded-2xl bg-[#1025420D]">
          {{ $t('reservation.voucher.deduction') }} Rp. {{ formattingNumber(scheduleStore?.discountDeductionNominal) }}
        </div>
              ...
              <div class="relative mt-8 mb-4">
                  ...
                  <Transition>
                    <div v-show="isSuccessState" class="text-xl font-normal text-[#16A34A] absolute bottom-[-40px] left-0 w-full">
                      {{ $t('reservation.voucher.deduction') }}  Rp. {{ formattingNumber(scheduleStore?.discountDeductionNominal) }}  <span class="absolute right-0">({{ countDownDuration }})</span>
                    </div>
                  </Transition>
