<script setup>
  ...
  import TheNavBar from './components/daytrans/TheNavBar.vue';  
  import TheNavBarV2 from './components/v2/TheNavBar';  
  import LetterKeyboard from './components/daytrans/LetterKeyboardApp.vue';
  ...
  const routeNoNavbar = ['thanks', 'idle', 'boarding'];
  const routeWithNetworkIndicator = ['idle', 'boarding', 'thanks'];
  const routeToIdle = ['schedule', 'boarding', 'printTicket'];
  ...
  const idlingReset = () => {
    if (routeToIdle.includes(currentRoute.value)) {
  ...
  /* Page Width Content */
  const contentPageWidth = () => {
    return Features[store.clientId].useV2 ? 'w-[760px] p-[32px]' : 'w-[757px] px-[56px]';
  };
...
<template>
  <div class="mx-auto relative">
    ...
    <div @click="idlingReset" @input="idlingReset" class="appWrapper mx-auto w-[1080px] flex relative">
        ...
        <!-- <div class=" absolute top-0 left-0 text-[green] text-2xl z-[100]">
          {{ idlingCountDown }}
        </div> -->
        <!-- <DevTool/> -->
        ...
        <TheNavBar v-if="!routeNoNavbar.includes($route.name) && !Features[store.clientId].useV2" />
        <TheNavBarV2
          v-if="!routeNoNavbar.includes($route.name) && Features[store.clientId].useV2"
        />
        <div
          :class="[ 
            !routeNoNavbar.includes($route.name) ? `bg-[${colors.primaryColor}]` : '',
            contentPageWidth()
          ]"
          class="  h-[1920px] relative"
        >
          <!-- <Home/> -->
          <Suspense>
